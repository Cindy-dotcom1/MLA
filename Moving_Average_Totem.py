...
data[data.columns[2]]=pd.to_numeric(data[data.columns[2]].str.replace("\u202f",""))
data[data.columns[3]]=pd.to_numeric(data[data.columns[3]].str.replace("\u202f",""))
...
data_total_real.append(data_daily_max.iloc[-1]["Vélos ce jour / Today's total"])

data_daily_total = pd.DataFrame(data_daily_max["Vélos ce jour / Today's total"])
