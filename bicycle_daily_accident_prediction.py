...
df_bikes_date = pd.DataFrame(df_bikes_sort.groupby("date").count()["identifiant accident"])
...
data = {"identifiant accident":0} # replace the number of accidents with 0 for the missing dates
df_bikes_2005 = df_bikes_2005.append(pd.DataFrame(data,index=datamissing)) 
df_bikes_2005 = df_bikes_2005.sort_index(axis = 0)
df_bikes_2005 = df_bikes_2005.rename(columns={"identifiant accident":"2005"}) # rename the column
...
df_bikes_2005['mean'] = df_bikes_2005.mean(axis=1) # df_bikes_2005['mean'] will be the input
...
fig = plt.figure(figsize=(16,9))
ax = plt.axes()
ax.plot(df_bikes_2018["identifiant accident"].values,"r.-",label="real_output")
